...
Base.prototype.tasks = function(room) {
    ...
    var terminal = room.terminal,
        ...
        workersWithEnergy = _.filter(workerList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] > 0).length > 0,
        workersWithMinerals = _.filter(workerList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] !== _.sum(c.carry)).length > 0,
        workersWithNothing = _.filter(workerList, (c) => !c.memory.currentTask && _.sum(c.carry) === 0).length > 0,

        collectorList = Cache.creeps[roomName] && Cache.creeps[roomName].collector || [],
        collectorsWithEnergy = _.filter(collectorList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] > 0).length > 0,
        collectorsWithNothing = _.filter(collectorList, (c) => !c.memory.currentTask && _.sum(c.carry) === 0).length > 0,
        
        storerList = Cache.creeps[roomName] && Cache.creeps[roomName].storer || [],
        storersWithEnergy = _.filter(storerList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] > 0).length > 0,
        storersWithMinerals = _.filter(storerList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] !== _.sum(c.carry)).length > 0,
        storersWithNothing = _.filter(storerList, (c) => !c.memory.currentTask && _.sum(c.carry) === 0).length > 0,

        scientistList = Cache.creeps[roomName] && Cache.creeps[roomName].scientist || [],
        scientistsWithEnergy = _.filter(scientistList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] > 0).length > 0,
        scientistsWithMinerals = _.filter(scientistList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] !== _.sum(c.carry)).length > 0,
        scientistsWithNothing = _.filter(scientistList, (c) => !c.memory.currentTask && _.sum(c.carry) === 0).length > 0,

        upgraderList = Cache.creeps[roomName] && Cache.creeps[roomName].upgrader || [],
        upgradersWithEnergy = _.filter(upgraderList, (c) => !c.memory.currentTask && c.carry[RESOURCE_ENERGY] > 0).length > 0,
        upgradersWithNothing = _.filter(upgraderList, (c) => !c.memory.currentTask && _.sum(c.carry) === 0).length > 0,
