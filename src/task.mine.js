...
Mine.prototype.run = function(creep) {
    ...
    if (container.pos.getRangeTo(creep) === 0) {
        ...
        if (_.filter(Cache.creepsInRoom("miner", creep.room), (c) => c.room.name === creep.room.name && c.memory.home === creep.room.name && c.memory.container === creep.memory.container && c.pos.getRangeTo(creep) === 1 && c.ticksToLive > creep.ticksToLive)) {
            creep.say(":(", true);
            creep.suicide();
        }
    }
