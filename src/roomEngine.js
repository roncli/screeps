...
class RoomEngine {
    ...
    checkSpawn(Role, canSpawn) {
        ...
        if (checkSettings.spawnFromRegion) {
            ({0: spawnToUse} = _.filter(Game.spawns, (s) => !Cache.spawning[s.id] && !s.spawning && s.room.memory && s.room.memory.roomType && s.room.memory.roomType.type === "base" && s.room.memory.region === room.memory.region && s.room.energyAvailable >= Utilities.getBodypartCost(spawnSettings.body)).sort((a, b) => (a.room.name === roomName ? 0 : 1) - (b.room.name === roomName ? 0 : 1)));
        } else {
            ({0: spawnToUse} = _.filter(Game.spawns, (s) => !Cache.spawning[s.id] && !s.spawning && s.room.memory && s.room.memory.roomType && s.room.memory.roomType.type === "base" && s.room.name === supportRoomName && s.room.energyAvailable >= Utilities.getBodypartCost(spawnSettings.body)));
        }
