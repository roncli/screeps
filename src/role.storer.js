var Cache = require("cache"),
    ...
    Storer = {
        ...
        assignTasks: (room, tasks) => {
            ...
            var creepsWithNoTask = _.filter(Utilities.creepsWithNoTask(Cache.creepsInRoom("storer", room)), (c) => !c.spawning && c.ticksToLive > 150),
                assigned = [];
            ...
            _.forEach(_.sortBy([].concat.apply([], [tasks.collectEnergy.storerTasks, tasks.collectMinerals.storerTasks]), (t) => -(t.object.energy || _.sum(t.object.store) || 0)), (task) => {
                ...
                var energy = (task.object.energy || _.sum(task.object.store) || 0) - _.reduce(Utilities.creepsWithTask(Game.creeps, {type: task.type, id: task.id}), function(sum, c) {return sum + (c.carryCapacity - _.sum(c.carry));}, 0);
                if (energy > 0) {
