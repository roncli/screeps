...
class RoleArmyRanged {
    ...
    static assignBuildingTasks(army) {
        const {creeps: {[army.name]: creeps}} = Cache,
            creepsWithNoTask = _.filter(Utilities.creepsWithNoTask(creeps && creeps.armyRanged || []), (c) => !c.spawning && (!c.memory.labs || c.memory.labs.length === 0) && c.memory.currentTask.priority !== Game.time);
