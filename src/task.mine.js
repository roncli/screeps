...
Mine.prototype.run = function(creep) {
    ...
    if (container.pos.x === creep.pos.x && container.pos.y === creep.pos.y && container.pos.roomName === creep.pos.roomName) {
        ...
        if (_.filter([].concat.apply([], [Cache.creeps[creep.room] && Cache.creeps[creep.room].miner || [], Cache.creeps[creep.room] && Cache.creeps[creep.room].remoteMiner || []]), (c) => c.room.name === creep.room.name && c.memory.container === creep.memory.container && c.pos.getRangeTo(creep) === 1 && c.ticksToLive > creep.ticksToLive && c.fatigue === 0).length > 0) {
            creep.say(":(", true);
            creep.suicide();
        }
    }
